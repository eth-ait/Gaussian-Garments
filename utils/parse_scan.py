...
def parse_scan(scan_data, source_path, fg_label=None, only_fg=True):
    ...
    # load camera_list and camera_params: ['0004', '0016', '0028', '0040', '0052', '0064', '0076', '0088']
    camera_list = RGB_CAMERAS
    ...
    # label_mesh = extract_label_meshes(scan_data.vertices, scan_data.faces, scan_labels, SURFACE_LABEL)
    # fg_mesh = trimesh.Trimesh(vertices=label_mesh[fg_label]['vertices'],
    #                         faces=label_mesh[fg_label]['faces'],
    #                         process=True)
...
def rasterized(th_verts, th_faces, source_path):
    camera_fn = os.path.join(source_path, 'cameras.json')

    # load mesh ply
    th_verts = th_verts.to(torch.float32).unsqueeze(0).cuda()
    th_faces = th_faces.to(torch.long).unsqueeze(0).cuda()
    scan_mesh = Meshes(th_verts, th_faces).cuda()
    ...
    # # -------------------- Rasterize -------------------- # #
    rasterize = {}
    for nc in tqdm.tqdm(range(len(RGB_CAMERAS)), desc="Rasterizing"):
        camera_id = RGB_CAMERAS[nc]
        # get capture_rasts from camera and scan_mesh
        capture_rasts = MeshRasterizer(cameras=camera_agents[camera_id], raster_settings=raster_settings)(scan_mesh)
        # get pix_to_face (H, W)
        pix_to_face = capture_rasts.pix_to_face[0].cuda()
        rasterize[camera_id] = pix_to_face
    return rasterize
